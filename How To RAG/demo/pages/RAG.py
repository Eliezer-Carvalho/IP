...
from code.llm import LOAD_MODEL, INFERENCE_MODEL_RAG
from code.rag import RAG
...
files =  st.file_uploader ("Selecione Documentos", type = ["pdf"], accept_multiple_files = True) #https://docs.streamlit.io/develop/api-reference/widgets/st.file_uploader
context = st.text_area ("Qual vai ser o contexto ?", height = 300)

save = st.button ("Guardar Informação")
...
if mensagens != None and SELECT_MODEL != None:
    ...
    rag = RAG (mensagens, st.session_state["files"])

    st.write (f"Contexto: {rag}")

    tokens = INFERENCE_MODEL_RAG (mensagens, st.session_state.tokenizer, st.session_state.modelo, st.session_state["context"], rag)
